// AI 서비스 통합 관리
import { JEJU_COURSE_DATA } from '../components/course/courseData';
import type { WeekData, PhaseData } from '../components/course/types';
...
export class AILearningService {
  ...
  private generateSystemPrompt(week: number, phase: number, mode: 'guided' | 'self-directed'): string {
    // 안전한 데이터 접근을 위한 null 체크 추가
    if (!JEJU_COURSE_DATA || !Array.isArray(JEJU_COURSE_DATA)) {
      console.warn('JEJU_COURSE_DATA is not available, using fallback data');
      return this.generateFallbackSystemPrompt(week, phase, mode);
    }

    const weekData = JEJU_COURSE_DATA.find(w => w && w.id === week);
    const phaseData = weekData?.phases?.find(p => p && p.id === phase);
...
- 학습 목표: ${phaseData?.guidedContent?.objective || phaseData?.selfDirectedContent?.objective || 'AI와 함께하는 실습 학습'}
...
**예시 대화 흐름:**
"좋습니다! 제주도 여행 계획의 첫 단계로 여행 목적을 명확히 해보겠습니다. 
다음 질문에 답해주세요:
1. 이번 제주도 여행의 주요 목적은 무엇인가요? (휴식/관광/체험/기타)
2. 함께 가는 사람은 누구인가요?
3. 선호하는 여행 스타일은 어떤가요?

이 질문들에 답하시면 맞춤형 여행 계획을 세워보겠습니다!"`;
    } else {
      return `${baseContext}
...
**예시 대화 흐름:**
"제주도 여행 계획을 스스로 세워보는 실습을 시작해보겠습니다!

어떤 방향으로 계획을 세워보고 싶으신가요? 예를 들어:
- 관심 지역부터 정하기
- 여행 테마부터 설정하기  
- 예산이나 일정부터 고려하기

어떤 방법이든 좋습니다. 여러분만의 접근 방식으로 시작해보세요. 
진행하면서 막히는 부분이 있으면 언제든 도움을 요청하세요!"`;
    }
  }

  // 폴백 시스템 프롬프트 생성 (데이터가 없을 때)
  private generateFallbackSystemPrompt(week: number, phase: number, mode: 'guided' | 'self-directed'): string {
    const baseContext = `
당신은 AI 협업을 가르치는 전문 교육 도우미입니다.
...
**🎯 가이드형 모드:**
단계별로 체계적인 안내를 제공하며, 구체적인 질문과 즉시 피드백을 통해 학습을 도와드립니다.
...
**🚀 자기주도형 모드:**
열린 질문과 방향 제시를 통해 학습자의 창의적 탐구를 지원합니다.
...
${week}주차 ${phase}페이즈 실습에 오신 것을 환영합니다! 어떤 방향으로 시작해보고 싶으신가요?`;
  ...
  private simulateAIResponse(
    ...
    if (JEJU_COURSE_DATA && Array.isArray(JEJU_COURSE_DATA)) {
      weekData = JEJU_COURSE_DATA.find(w => w && w.id === session.week);
      phaseData = weekData?.phases?.find(p => p && p.id === session.phase);
    }
    ...
    if (session.mode === 'guided') {
      return this.generateGuidedResponse(userMessage, session, isFirstMessage, message, phaseData);
    } else {
      return this.generateSelfDirectedResponse(userMessage, session, isFirstMessage, message, phaseData);
    }
  }
...
**📋 실습 목표**
${phaseData?.guidedContent?.objective || '단계별 체계적 학습을 통한 목표 달성'}
...
**🎯 현재 진행 상황**
- 목표: ${phaseData?.guidedContent?.objective || 'AI와 함께하는 실습 학습'}
...
**🎯 실습 목표**
${phaseData?.selfDirectedContent?.objective || '자기주도적 학습을 통한 창의적 문제 해결'}
